...
const workspaceRoot = 'C:\\Data\\data';
const toolRoot = path.join(workspaceRoot, 'dalat-tiktok-carousel-tool', 'backend');
const workbookPath = path.join(workspaceRoot, 'F&B ĐÀ LẠT.xlsx');
const imageMappingPath = path.join(toolRoot, 'image-mapping.json');
const reportPath = path.join(toolRoot, 'mapping-report.md');
