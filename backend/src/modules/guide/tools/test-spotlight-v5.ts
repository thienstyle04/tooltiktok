...
function item(sectionKey: SectionKey, n: number, isPartner = false): GuideItem { const id = `${sectionKey}-${n}`; const imageUrl = `https://example.invalid/${id}.jpg`; return { id, sectionKey, sectionTitle: sectionKey, name: `Địa điểm ${n} ${sectionKey}`, address: `${n} Ngô Quyền, Cam Ly - Đà Lạt`, type: sectionKey, openHours:'', style:'', highlight:'', partnerFlag:isPartner ? 'X' : '', isPartner, headPrice:'', hasHeadPriceColumn:false, price:'', phone:'', imageUrl, imageMapped:true, imageMappingKey:id, imageSource:'manual', candidateImageUrls:[imageUrl] }; }
const partnerKeys = new Set(['quan_an-1','quan_an-2','quan_an-3','cafe-1','cafe-2','khu_du_lich-1','khu_du_lich-2']);
const itemsBySection = Object.fromEntries(keys.map((key) => [key, key === 'homestay' || key === 'dich_vu' ? [] : Array.from({length: key === 'quan_an' ? 5 : 2}, (_,i) => item(key, i+1, partnerKeys.has(`${key}-${i+1}`)))])) as WorkbookItemsBySection;
...
assert.equal(places.filter((x:any) => x.isPartner).length, 7, 'V5 phải có đúng 7/13 địa điểm đối tác khi pool đủ');
assert.equal(new Set(places.filter((x:any) => x.isPartner).map((x:any) => x.rawName)).size, 7, 'File partners phải nhận đủ 7 tên đối tác khác nhau');
