...
async function main() {
  ...
  try {
    ...
    assert.equal(partialRetry.status().sources[0].lastPublishedAt, new Date(now).toISOString());
    assert.match(partialRetry.status().sources[0].progress!.stage, /Đã công bố/);
    ...
    await partialRetry.tick();
    assert.equal(partialRetry.status().report?.sources?.dalat.phase, 'complete');
    const reportBeforeManual = JSON.stringify(partialRetry.status().report);
    await partialRetry.manual('dalat');
    assert.equal(JSON.stringify(partialRetry.status().report), reportBeforeManual, 'Daytime sync must not rewrite last night report');
    const fresh = new NightSyncCoordinator({ ...options, file: path.join(dir, 'no-night.json') });
    await fresh.tick();
    assert.deepEqual(fresh.status().report?.sources, {});
    assert.match(fresh.status().report!.message, /không có bản ghi/);
    const interruptedFile = path.join(dir, 'interrupted.json');
    fs.writeFileSync(interruptedFile, JSON.stringify({ sources: { dalat: { initialized: true, phase: 'running', progress: { stage: 'Đang công bố dữ liệu mới' } } } }));
    const interrupted = new NightSyncCoordinator({ ...options, file: interruptedFile });
    assert.equal(interrupted.status().sources[0].phase, 'partial');
    assert.match(interrupted.status().sources[0].progress!.stage, /bị ngắt/);
    assert.equal(interrupted.status().sources[0].lastPublishedAt, undefined);
