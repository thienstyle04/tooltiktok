...
for(const page of pages) {
 ...
 assert.ok(page.items.filter(item=>item.isPartner).length<=2);
...
assert.throws(()=>buildItineraryNotePages({itemsBySection:{...pools,quan_an:[]}},'missing'),/không đủ/);
const normal=Object.fromEntries(keys.map(key=>[key,pools[key].map(item=>({...item,isPartner:false}))])) as WorkbookItemsBySection;
assert.equal(buildItineraryNotePages({itemsBySection:normal},'normal').length,2);
const duplicate={...normal,cafe:normal.quan_an.slice(0,2).map(item=>({...item,sectionKey:'cafe' as SectionKey}))};
...
console.log('PASS itinerary note: 2 days, category quotas, partners, unique places, original addresses, no images, timezone and missing data.');
