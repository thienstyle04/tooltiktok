...
export default function PreviewPanel({ deck, list, selectedPageIndex, onPageSelect, onDeleteList, loading, coverImageUrls = [] }) {
  ...
  if (!list) {
    const emptyMessage = deck.id === 'spotlight-v6-green' || deck.id === 'spotlight-v6-dark' || deck.id === 'spotlight-v6-persimmon'
      ? `Mẫu chưa có list. Mở Caption AI, chọn số lượng và bấm “Tạo ${deck.navTitle}” để dựng list đầu tiên.`
